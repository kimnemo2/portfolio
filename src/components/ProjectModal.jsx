...
export default function ProjectModal({ project, onClose }) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const check = () => setShowArrow(el.scrollHeight - el.scrollTop - el.clientHeight > 40);
    check();
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Scrollable area */}
          <div
            ref={scrollRef}
            className="no-scrollbar overflow-y-auto max-h-[85vh]"
          ...
          >
            ...
            <div className="px-6 py-6 space-y-6">
              {details.links && details.links.length > 0 && (
                <div>
                  <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-2 uppercase tracking-wide">🔗 바로가기</h3>
                  <div className="flex flex-wrap gap-2">
                    {details.links.map((link, i) => (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-brand-200 dark:border-brand-800 text-sm text-brand-600 dark:text-brand-400 hover:bg-brand-50 dark:hover:bg-brand-900/20 transition-colors"
                      >
                        {link.label}
            ...
            {showArrow && (
              ...
              >
                <motion.div
                  ...
                  className="text-gray-400/50 dark:text-gray-500/50 text-sm"
                >
                  ▼
                </motion.div>
              </motion.div>
